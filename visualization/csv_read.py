...
def main():
    ...
    raw_pd_data = pd.read_csv("../data/data_1400/data_1400.csv", header=None, names=fieldnames)
    ...
    times_ = np.array(pd_data['date'][t1:t2:dt])
    fig = pl.figure()
    t_ = range(len(times_))
    ...
    t = range(len(times))
    pl.xticks(t[0::6000], dates[0::6000], rotation='vertical')
    pl.plot(t, temp, '-r', label="Temp, C")
    # pl.plot(t, fr_fw, '-b', label="FARred/FARwhite")
    # pl.plot(t, hum, '-b', label="Humidity, %")
    # pl.plot(t, air * 100, '-k', label="Airflow ON")
